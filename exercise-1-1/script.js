...
async function playPhrase(text, voiceName) {
    ...
    // For this exercise, we want to disable all words, not just dialogue lines
    const allPronunciationWords = document.querySelectorAll('.pronunciation-word');

    // Disable all words during loading/playback
    allPronunciationWords.forEach(word => word.style.pointerEvents = 'none');
    ...
    try {
        ...
        const apiKey = "AIzaSyBchivn4oOSIMzAUT1RSCbjlyh7937-gdA"; // IMPORTANT: Replace with your actual API key
        ...
        if (audioData && mimeType && mimeType.startsWith("audio/L16")) {
            ...
            // When audio ends, re-enable words
            audio.onended = () => {
                allPronunciationWords.forEach(word => word.style.pointerEvents = 'auto');
            };
        ...
        // Ensure words are re-enabled even if there's an error during playback start
